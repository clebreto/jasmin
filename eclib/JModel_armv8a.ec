...
op nzcv_32 (r: W32.t) (u s: int) : bool * bool * bool * bool =
  (W32.msb r,
   r = W32.zero,
   to_uint r <> u,
   to_sint r <> s).
...
abbrev with_nzcv_32 r u s =
  let (n, z, c, v) = nzcv_32 r u s in
  (n, z, c, v, r).
...
op ADDS_32 (x y: W32.t) : bool * bool * bool * bool * W32.t =
  with_nzcv_32 (x + y) (to_uint x + to_uint y) (to_sint x + to_sint y).

op ADC_64 (x y: W64.t) (c: bool) : W64.t =
  x + y + (if c then W64.one else W64.zero).
op ADC_32 (x y: W32.t) (c: bool) : W32.t =
  x + y + (if c then W32.one else W32.zero).
...
op ADCS_32 (x y: W32.t) (c: bool) : bool * bool * bool * bool * W32.t =
  let r = ADC_32 x y c in
  with_nzcv_32 r (to_uint x + to_uint y + b2i c) (to_sint x + to_sint y + b2i c).

op SUB_64 (x y: W64.t) : W64.t = x - y.
op SUB_32 (x y: W32.t) : W32.t = x - y.
...
op SBC_32 (x y: W32.t) (c: bool) : W32.t = ADC_32 x (invw y) c.
...
op UBFX_32 (x: W32.t) (lsb width: W8.t) : W32.t =
  (x `<<` W8.of_int (32 - to_uint width - to_uint lsb))
    `>>` W8.of_int (32 - to_uint width).

op SBFX_64 (x: W64.t) (lsb width: W8.t) : W64.t =
  (x `<<` W8.of_int (64 - to_uint width - to_uint lsb))
    `|>>` W8.of_int (64 - to_uint width).
op SBFX_32 (x: W32.t) (lsb width: W8.t) : W32.t =
  (x `<<` W8.of_int (32 - to_uint width - to_uint lsb))
    `|>>` W8.of_int (32 - to_uint width).
...
op REV_32 (x: W32.t) : W32.t =
  W32.init (fun i => x.[8 * (3 - i %/ 8) + i %% 8]).

op REV16_64 (x: W64.t) : W64.t =
  W64.init (fun i => x.[16 * (i %/ 16) + 8 * (1 - (i %% 16) %/ 8) + i %% 8]).
op REV16_32 (x: W32.t) : W32.t =
  W32.init (fun i => x.[16 * (i %/ 16) + 8 * (1 - (i %% 16) %/ 8) + i %% 8]).
